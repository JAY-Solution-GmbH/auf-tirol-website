...
# We want to inject our custom styles right before </head>
custom_styles = """
  <style>
    .legal-page { padding: 80px 0; }
    .legal-card { background: var(--panel); border: 1px solid var(--border); border-radius: var(--radius-xl); box-shadow: var(--shadow); padding: 40px; margin-bottom: 40px; }
    .legal-content h1 { font-size: 38px; margin: 0 0 30px; letter-spacing: -0.02em; }
    .legal-content h2 { font-size: 26px; margin: 40px 0 16px; border-bottom: 1px solid var(--border); padding-bottom: 12px; }
    .legal-content h3 { font-size: 20px; margin: 30px 0 12px; }
    .legal-content p, .legal-content li { color: var(--muted); margin-bottom: 16px; font-size: 16px; line-height: 1.7; }
    .legal-content a { color: var(--accent); text-decoration: underline; text-underline-offset: 4px; }
    .legal-content a:hover { color: var(--text); }
    .legal-content table { width: 100%; border-collapse: collapse; margin-top: 20px; margin-bottom: 30px; }
    .legal-content th, .legal-content td { text-align: left; padding: 14px 16px; border: 1px solid var(--border); color: var(--muted); font-size: 15px; }
    .legal-content th { background: rgba(255,255,255,0.05); color: var(--text); font-weight: 600; }
    .legal-content td { background: var(--panel-strong); }
    .back-btn-wrap { margin-bottom: 30px; }
    .back-btn { display: inline-flex; align-items: center; gap: 8px; color: var(--text); text-decoration: none; font-weight: 500; padding: 12px 24px; border: 1px solid var(--border); border-radius: var(--radius-lg); background: var(--panel); transition: 0.2s ease; font-size: 15px; }
    .back-btn:hover { background: var(--panel-strong); border-color: rgba(255,255,255,0.2); transform: translateY(-1px); }
  </style>
"""

head_content = head_content.replace('</head>', custom_styles + '\n</head>')

# Extract footer block
footer_match = re.search(r'(<footer>.*?</footer>\s*</body>\s*</html>)', content, re.DOTALL)
if footer_match:
...
body_start = """
<body style="background: radial-gradient(circle at top, rgba(255,255,255,0.08), transparent 28%), linear-gradient(180deg, rgba(255,255,255,0.02), transparent 18%), var(--bg);">
  <main class="legal-page container">
    <div class="back-btn-wrap">
      <a href="auf-tirol-site_index.html" class="back-btn">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="19" y1="12" x2="5" y2="12"></line><polyline points="12 19 5 12 12 5"></polyline></svg>
        Zurück zur Startseite
      </a>
    </div>
    <div class="legal-card legal-content">
"""
...
impressum_body = """
    ...
    <p><strong>Dusan Djokic</strong><br>
    [Adresse, PLZ, Ort]</p>
    ...
    <strong>Web:</strong> <a href="https://www.jay-solution.com" target="_blank">www.jay-solution.com</a></p>
    ...
    <p>Die Bilder, Fotos und Grafiken auf dieser Webseite sind urheberrechtlich geschützt. Die Verwertungsrechte liegen bei: Dusan Djokic</p>
...
with open("impressum.html", "w") as f:
    f.write(head_content.replace("<title>AUF Tirol – Aktiv und Fit Tirol</title>", "<title>Impressum – AUF Tirol</title>") + body_start + impressum_body + body_end + footer_html)

with open("datenschutz.html", "w") as f:
    f.write(head_content.replace("<title>AUF Tirol – Aktiv und Fit Tirol</title>", "<title>Datenschutz – AUF Tirol</title>") + body_start + datenschutz_body + body_end + footer_html)
